...
import '../../Styles/Women/WomenDetail.css';
...
class WomenDetails extends react.Component {
    constructor() {
        super();
        this.state = {
            Itemdetail: {},
    ...
    componentDidMount() {
        ...
        axios({
            url: `https://amazon-clone-db.herokuapp.com/fashionwomen/details/${productId}`,
            Headers: {
                'content-type': 'application/json'
            },
            method: 'GET'
        }).then(res => this.setState({
            Itemdetail: res.data.fashion_women_det_prodID,
            Itemdescription: res.data.fashion_women_det_prodID.small_description.split('\n'),
        })).catch(err => console.log(err))
        axios({
            url: `https://amazon-clone-db.herokuapp.com/fashionwomen/basic/${productId}`,
            Headers: {
                'content-type': 'application/json'
            },
            method: 'GET'
        }).then(res => this.setState({
            ItembasicDetail: res.data.fashion_women_basic_detail_prodID
        })).catch(err => console.log(err))
    ...
    render() {
        ...
        return (
            ...
            <div className="container-fluid">
                <div className="row">
                    <div className="women-item-img col-5 col-sm-4 col-md-4 col-lg-4 col-xl-4">
                            ...
                            {Itemdetail && Itemdetail.images && Itemdetail.images.map((item) => {
                                return (
                                    <div className="item-detail-gallery-image">
                                        <img src={item} alt="Women" />
                                    </div>
                                )
                            })}
                        </Carousel>
                    </div>
                    <div className="women-item-detail col-7 col-sm-6 col-md-6 col-lg-6col-xxl-6">
                        <div className="women-item-name">
                            {Itemdetail.name}
                        </div>
                        <div className="women-item-brand-url">
                            <a href={Itemdetail.brand_url}>{Itemdetail.brand}</a>
                        </div>
                        <div className="women-item-rating">
                            {Itemdetail.average_rating} rating | {Itemdetail.total_reviews} reviews | {Itemdetail.total_answered_questions} + answered questions
                        </div>
                        {ItembasicDetail.is_amazon_choice === true ? <img className="women-item-amazon-choice" src="https://storage.googleapis.com/mw-imagelibrary/amazon-choice.png"></img> : null}
                        {ItembasicDetail.is_limited_deal === true ? <img className="women-item-amazon-limited-deal" src="https://www.pngall.com/wp-content/uploads/2016/06/Limited-offer-Free-Download-PNG.png"></img> : null}
                        <hr className="item-detail-split" />
                        <div className="women-item-price-deliver">
                            <div className="women-item-price">
                                Price: {ItembasicDetail.price_string}
                            </div>
                            <div className="women-item-has-prime">
                            ...
                            <div className="women-item-deliver">
                    ...
                    <div className="women-cart-buy col-12 col-sm-2 col-md-2 col-lg-2 col-xl-2">
...
export default WithRouter(WomenDetails);